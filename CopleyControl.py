...
class CopleyControl (PyTango.Device_4Impl):
    ...
    def read_Base_rate(self, attr):
        self.debug_stream("In read_Base_rate()")
        #----- PROTECTED REGION ID(CopleyControl.Base_rate_read) ENABLED START -----#
        attr.set_value(self.attr_Base_rate_read)
        
        #----- PROTECTED REGION END -----#	//	CopleyControl.Base_rate_read
        
    def write_Base_rate(self, attr):
        self.debug_stream("In write_Base_rate()")
        data = attr.get_write_value()
        #----- PROTECTED REGION ID(CopleyControl.Base_rate_write) ENABLED START -----#
        
        #----- PROTECTED REGION END -----#	//	CopleyControl.Base_rate_write
    ...
    def dev_status(self):
        """ 
        Gets the device status (stored in its device_status data member) and returns it to the caller.
        ...
        if int(argin) in HomeReferenceNrList:
            ...
            if int(argin) in [514, 530]:
                ...
                if self.checkHomeStatus():
                    ...
                    self.getValue(str(self.NodeID) + " s r0xc2 " + str(int(argin)))
                    self.getValue(str(self.NodeID) + " t 2")   
    ...
    def connectSerial(self):    
        ...
        try:
            dev = PyTango.DeviceProxy("pyserial/copleyctrl/1")            
            if dev.State() == PyTango.DevState.OFF:
                ...
                print "Device State of tango device pyserial/copleyctrl/1 Unknown"
        except:
            print("An exception with connecting the tango device pyserial/copleyctrl/1 occurred. ")      
        
    def getNodeID(self):
        """ 
        Get the node id from the device properties.
        """
        db = PyTango.Database()
    ...
    def getMaxVelocity(self):
        """ 
        Get the max Velocity from the device properties.
        """
        db = PyTango.Database()
